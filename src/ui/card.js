...
// Tailles réduites (voir AGENDA.md -- "icônes du classement trop grosses") : appliqué UNE fois
// ici, se répercute automatiquement sur tous les écrans qui réutilisent rankGlyph() (Panthéon,
// classement de défi entre amis, classements mondiaux) sans rien toucher ailleurs.
export function rankGlyph(i){
  if(i===0) return `<span class="rk-medal" title="${t('hof.rank1')}">${crownIcon(23)}</span>`;
  if(i===1) return `<span class="rk-medal" title="${t('hof.rank2')}">${medalIcon('Argent',21)}</span>`;
  if(i===2) return `<span class="rk-medal" title="${t('hof.rank3')}">${medalIcon('Bronze',21)}</span>`;
