...
async function main() {
  ...
  const a = run(['submit', challengeId, 'Alice', '150']);
  ...
  const b = run(['submit', challengeId, 'Bob', '220']);
  ...
  const fetchA = run(['fetch', challengeId]);
  ...
  const fetchB = run(['fetch', challengeId]);
  ...
  const aBetter = run(['submit', challengeId, 'Alice', '300']);
  check('une resoumission avec un MEILLEUR score est acceptée', aBetter.ok);
  const fetchAfterBetter = run(['fetch', challengeId]);
